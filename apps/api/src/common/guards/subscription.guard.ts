...
export const RequireSubscriptionFeature = (feature: string) => SetMetadata(SUBSCRIPTION_FEATURE_KEY, feature);

// Decorator to require minimum plan tier
export const RequirePlanTier = (...tiers: string[]) => SetMetadata(PLAN_TIER_KEY, tiers);
...
export class SubscriptionGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        const requiredTiers = this.reflector.getAllAndOverride<string[]>(PLAN_TIER_KEY, [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        if (!subscription) {
            throw new ForbiddenException('No active subscription found. Please contact your administrator.');
        }
        ...
        if (status === 'PAST_DUE') {
            throw new ForbiddenException('Your subscription payment is past due. Please update your payment method.');
        }
