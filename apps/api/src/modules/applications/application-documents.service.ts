...
import { CreateApplicationDocumentDto, ReviewDocumentDto, UpdateApplicationDocumentDto } from './dto/application-document.dto';
...
export class ApplicationDocumentsService {
    ...
    async requestDocument(applicationId: string, dto: CreateApplicationDocumentDto, tenantId: string) {
    ...
    async reviewDocument(documentId: string, dto: ReviewDocumentDto, reviewerId: string, tenantId: string) {
    ...
    async updateDocument(documentId: string, dto: UpdateApplicationDocumentDto, tenantId: string) {
        const document = await this.prisma.applicationDocument.findFirst({
            where: {
                id: documentId,
                application: {
    ...
    async getDocumentStats(applicationId: string) {
        ...
        return {
            ...
            uploaded: documents.filter(d => ['UPLOADED', 'PENDING_REVIEW', 'APPROVED', 'REJECTED'].includes(d.status)).length,
            pendingReview: documents.filter(d => d.status === 'PENDING_REVIEW').length,
            approved: documents.filter(d => d.status === 'APPROVED').length,
            rejected: documents.filter(d => d.status === 'REJECTED').length,
            allRequiredApproved: documents.filter(d => d.isRequired).every(d => d.status === 'APPROVED'),
        };
    }
}
