...
export class AutoRejectionService {
    ...
    async updateConfig(tenantId: string, config: Partial<RejectionConfig>): Promise<RejectionConfig> {
    ...
    async getTemplates(tenantId: string, stage?: string): Promise<RejectionTemplate[]> {
        ...
        let templates = settings.map(s => s.value as unknown as RejectionTemplate);

        if (stage) {
            templates = templates.filter(t => t.stage === stage || t.stage === 'ALL');
        }

        return templates.sort((a, b) => a.stage.localeCompare(b.stage));
    }

    async getTemplate(tenantId: string, templateId: string): Promise<RejectionTemplate | null> {
        const setting = await this.prisma.setting.findUnique({
            where: { tenantId_key: { tenantId, key: `${REJECTION_TEMPLATE_KEY}_${templateId}` } },
        });
    ...
    async sendRejection(
        ...
        const body = this.personalizeContent(
            options?.customMessage || template?.body || this.getDefaultBody(application.status),
            application,
        );

        // Determine send time
        const delayHours = options?.immediate ? 0 : (template?.delayHours || config.defaultDelayHours);
        const sendAt = new Date(Date.now() + delayHours * 60 * 60 * 1000);

        // Check weekend restriction
        if (!config.sendOnWeekends && (sendAt.getDay() === 0 || sendAt.getDay() === 6)) {
            // Push to Monday
    ...
    ): Promise<{ total: number; sent: number; scheduled: number; failed: number }> {
        let sent = 0;
        let scheduled = 0;
        let failed = 0;
...
`;
    ...
    private async logRejection(application: any, status: 'SENT' | 'SCHEDULED' | 'FAILED'): Promise<void> {
        await this.prisma.activityLog.create({
            data: {
...
<p>Best regards,<br>{{company.name}} Recruiting Team</p>`,
        ...
        for (const preset of presets) {
            try {
                ...
                this.logger.warn(`Failed to create preset template ${preset.name}:`, error);
            }
        }
