...
export class CheckrService {
  ...
  async getCandidateReports(
    ...
    const response = await fetch(
      `${baseUrl}/candidates/${candidateId}/reports`,
      {
        headers: {
          Authorization: `Basic ${Buffer.from(`${config.apiKey}:`).toString("base64")}`,
        },
  ...
  async getPackages(
    config: CheckrConfig,
  ): Promise<{ id: string; name: string; screenings: string[] }[]> {
    const baseUrl = this.getBaseUrl(config.sandboxMode || false);
      ...
      return [
        {
          id: "tasker_standard",
          name: "Basic",
          screenings: ["ssn_trace", "national_criminal_search"],
        },
        {
          id: "driver_pro",
          name: "Standard",
  ...
  async testConnection(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          success: false,
          message: `API authentication failed: ${error}`,
        };
      }
    ...
    return [
      {
        id: "ssn_trace",
        name: "SSN Trace",
        description: "Verifies SSN and returns address history",
      },
      {
        id: "national_criminal_search",
        name: "National Criminal Search",
        description: "Searches national criminal databases",
      },
      {
        id: "county_criminal_search",
        name: "County Criminal Search",
        description: "Searches county court records",
      },
      {
        id: "federal_criminal_search",
        name: "Federal Criminal Search",
        description: "Searches federal court records",
      },
      {
        id: "motor_vehicle_report",
        name: "Motor Vehicle Report",
        description: "Driving history and license verification",
      },
      {
        id: "education_verification",
        name: "Education Verification",
        description: "Verifies degrees and attendance",
      },
      {
        id: "employment_verification",
        name: "Employment Verification",
        description: "Verifies past employment",
      },
      {
      ...
      },
      {
        ...
        description: "Searches sex offender registries",
      },
      {
        id: "global_watchlist_search",
        name: "Global Watchlist",
        description: "Searches international watchlists",
      },
    ];
  }
}
