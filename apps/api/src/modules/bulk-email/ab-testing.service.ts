...
export class ABTestingService {
    ...
    async createTest(
        ...
        if (!dto.variants || dto.variants.length < 2) {
            throw new BadRequestException('At least 2 variants are required for A/B testing');
        }
        ...
        const variants: ABVariant[] = dto.variants.map((v, index) => {
            const percentage = index === dto.variants.length - 1 
                ? remainingPercentage 
    ...
    async startTest(tenantId: string, testId: string, userId: string): Promise<ABTest> {
        const test = await this.getTest(tenantId, testId);
    ...
    private async resolveRecipients(tenantId: string, test: ABTest): Promise<string[]> {
    ...
    private distributeRecipients(
        ...
        for (const variant of variants) {
            ...
            assignments[variant.id] = shuffled.slice(currentIndex, currentIndex + count);
            currentIndex += count;
        }
    ...
    private async sendVariantEmails(
        ...
        for (const candidate of candidates) {
            try {
                const personalizedSubject = this.personalize(variant.subject, candidate);
                ...
                this.logger.error(`Failed to send A/B test email to ${candidate.email}:`, error);
            }
        }
    }
    ...
    async getTestResults(tenantId: string, testId: string): Promise<{
        test: ABTest;
        results: {
            variantId: string;
            variantName: string;
        ...
        const results = test.variants.map(variant => {
            const stats = test.stats[variant.id] || { sent: 0, opened: 0, clicked: 0, replied: 0 };
            return {
                variantId: variant.id,
        ...
        let recommendation = 'Test is still running. Results will be available after completion.';
