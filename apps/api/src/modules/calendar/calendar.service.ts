...
export class CalendarService {
     ...
     */
    private async getGoogleConfig(tenantId: string): Promise<GoogleCalendarConfig | null> {
    ...
    async saveGoogleConfig(tenantId: string, config: GoogleCalendarConfig) {
        return this.prisma.setting.upsert({
            ...
            create: { tenantId, key: GOOGLE_CALENDAR_CONFIG_KEY, value: config as any, category: 'INTEGRATION', isPublic: false },
        });
    }
    ...
    async saveOutlookConfig(tenantId: string, config: OutlookCalendarConfig) {
        return this.prisma.setting.upsert({
            where: { tenantId_key: { tenantId, key: OUTLOOK_CALENDAR_CONFIG_KEY } },
            update: { value: config as any, category: 'INTEGRATION' },
            create: { tenantId, key: OUTLOOK_CALENDAR_CONFIG_KEY, value: config as any, category: 'INTEGRATION', isPublic: false },
        });
    }
     ...
     */
    async getAuthUrl(provider: CalendarProvider, userId: string, tenantId: string): Promise<string> {
        ...
        if (provider === 'GOOGLE') {
            ...
            if (!config?.clientId) throw new BadRequestException('Google Calendar not configured. Please add credentials in Settings.');
            return this.googleCalendar.getAuthUrl(config, state);
        } else if (provider === 'OUTLOOK') {
            const config = await this.getOutlookConfig(tenantId);
            if (!config?.clientId) throw new BadRequestException('Outlook Calendar not configured. Please add credentials in Settings.');
     ...
     */
    async connectCalendar(userId: string, tenantId: string, dto: ConnectCalendarDto) {
        ...
        try {
            if (provider === 'GOOGLE') {
                const config = await this.getGoogleConfig(tenantId);
                if (!config?.clientId) throw new BadRequestException('Google Calendar not configured');
                ...
                const config = await this.getOutlookConfig(tenantId);
                if (!config?.clientId) throw new BadRequestException('Outlook Calendar not configured');
     ...
     */
    private async getValidAccessToken(connection: any, tenantId: string): Promise<string> {
        ...
        if (!connection.refreshToken) {
            throw new BadRequestException('Calendar connection needs re-authorization');
        }
        ...
        if (connection.provider === 'GOOGLE') {
            const config = await this.getGoogleConfig(tenantId);
            if (!config) throw new BadRequestException('Google Calendar not configured');
            tokens = await this.googleCalendar.refreshAccessToken(config, connection.refreshToken);
        } else if (connection.provider === 'OUTLOOK') {
            const config = await this.getOutlookConfig(tenantId);
            if (!config) throw new BadRequestException('Outlook Calendar not configured');
     ...
     */
    async getFreeBusy(userId: string, tenantId: string, dto: FreeBusyQueryDto): Promise<UserAvailability[]> {
     ...
     */
    async findCommonAvailability(userId: string, tenantId: string, dto: FreeBusyQueryDto): Promise<AvailabilitySlot[]> {
        ...
        for (let i = 1; i < userAvailabilities.length; i++) {
            ...
            commonSlots = commonSlots.filter(common =>
                userSlots.some(slot =>
                    slot.start === common.start && slot.end === common.end
                )
            );
        }
