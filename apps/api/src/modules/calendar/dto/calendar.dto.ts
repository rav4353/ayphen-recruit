...
export class CreateCalendarEventDto {
    ...
    @ApiPropertyOptional({ description: 'Video meeting link (Zoom, Meet, Teams)' })
    @IsOptional()
    @IsString()
    meetingLink?: string;

    @ApiPropertyOptional({ type: [String], description: 'List of attendee emails' })
    @IsOptional()
    @IsArray()
    @IsString({ each: true })
    attendees?: string[];
...
export class FreeBusyQueryDto {
    @ApiProperty({ type: [String], description: 'User IDs to check availability for' })
    @IsArray()
    @IsString({ each: true })
    userIds: string[];
