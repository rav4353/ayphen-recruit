...
export class GoogleCalendarService {
     ...
     */
    async exchangeCodeForTokens(config: GoogleCalendarConfig, code: string, redirectUri?: string): Promise<GoogleTokens> {
     ...
     */
    async refreshAccessToken(config: GoogleCalendarConfig, refreshToken: string): Promise<GoogleTokens> {
        const response = await fetch('https://oauth2.googleapis.com/token', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: new URLSearchParams({
     ...
     */
    async createEvent(accessToken: string, event: GoogleEvent, calendarId = 'primary'): Promise<GoogleEvent> {
     ...
     */
    async updateEvent(accessToken: string, eventId: string, event: Partial<GoogleEvent>, calendarId = 'primary'): Promise<GoogleEvent> {
        const response = await fetch(
     ...
     */
    async deleteEvent(accessToken: string, eventId: string, calendarId = 'primary'): Promise<void> {
        const response = await fetch(
            `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events/${eventId}`,
            {
                method: 'DELETE',
     ...
     */
    async getFreeBusy(accessToken: string, emails: string[], timeMin: string, timeMax: string): Promise<FreeBusyResponse> {
     ...
     */
    async listEvents(accessToken: string, calendarId = 'primary', timeMin?: string, timeMax?: string): Promise<GoogleEvent[]> {
        const params = new URLSearchParams({
            singleEvents: 'true',
            orderBy: 'startTime',
            ...(timeMin && { timeMin }),
