...
export class CandidateDedupService {
     ...
     */
    async findDuplicates(tenantId: string, options?: {
        minScore?: number;
        limit?: number;
        ...
        for (let i = 0; i < candidates.length; i++) {
            ...
            if (duplicates.length > 0) {
                ...
                duplicateGroups.push({
                    ...
                    totalMatchScore: duplicates.reduce((sum, d) => sum + d.matchScore, 0) / duplicates.length,
                });
            }
        ...
        return duplicateGroups.sort((a, b) => b.totalMatchScore - a.totalMatchScore);
    }
    ...
    async checkForDuplicates(
        tenantId: string,
        candidateData: { email: string; firstName?: string; lastName?: string; phone?: string },
    ): Promise<DuplicateCandidate[]> {
        ...
        return potentialDuplicates.map(c => {
            const { score, reasons } = this.calculateMatchScore(candidateData as any, c);
            return {
                candidateId: c.id,
    ...
    private calculateMatchScore(
        ...
        if (candidate1.email && candidate2.email) {
            if (candidate1.email.toLowerCase() === candidate2.email.toLowerCase()) {
                ...
                if (domain1 === domain2 && this.stringSimilarity(local1, local2) > 0.8) {
                    score += 25;
        ...
        if (candidate1.phone && candidate2.phone) {
            ...
            } else if (phone1.slice(-10) === phone2.slice(-10) && phone1.length >= 10) {
                score += 20;
        ...
        // Name match
        const name1 = `${candidate1.firstName || ''} ${candidate1.lastName || ''}`.toLowerCase().trim();
        const name2 = `${candidate2.firstName || ''} ${candidate2.lastName || ''}`.toLowerCase().trim();
