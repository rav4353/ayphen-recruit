...
export class CandidateScoringService {
  ...
  async scoreCandidate(
    ...
    const missingSkills = jobSkills.filter(
      (s) => !candidateSkills.some((cs) => cs.includes(s) || s.includes(cs)),
    );

    const { highlights, concerns } = this.generateInsights(
      candidate,
      job,
      breakdown,
    );
    ...
    const results = await Promise.all(
      candidates.map(async (candidate) => {
        ...
        return {
          ...
          averageScore:
            scoredJobs > 0 ? Math.round(totalScore / scoredJobs) : 0,
  ...
  private extractRequiredYears(job: any): number {
    const text =
      `${job.description || ""} ${job.requirements || ""}`.toLowerCase();
  ...
  private calculateEducationMatch(candidate: any, job: any): number {
    ...
    const hasPhd = candidateEducation.some(
      (e: any) =>
    ...
    const hasBachelors = candidateEducation.some(
      (e: any) =>
    ...
    const requiresPhd =
      jobText.includes("phd") || jobText.includes("doctorate");
    const requiresMasters = jobText.includes("master");
    const requiresBachelors =
      jobText.includes("bachelor") || jobText.includes("degree");
