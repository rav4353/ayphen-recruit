...
export class CandidateTaggingService {
        ...
        if (tagsToAdd.length > 0) {
            ...
            await this.prisma.activityLog.create({
                data: {
                    ...
                    candidateId,
                    metadata: { tagsAdded: tagsToAdd, rulesApplied: activeRules.map(r => r.name) },
                },
            });
        }
     ...
     */
    private async tagCandidateWithRules(candidateId: string, rules: TagRule[]): Promise<string[]> {
        const candidate = await this.prisma.candidate.findUnique({
            where: { id: candidateId },
        });
    ...
    private evaluateCondition(candidate: any, condition: TagCondition): boolean {
        ...
        switch (field) {
            case 'skills':
                fieldValue = (candidate.skills || []).map((s: string) => s.toLowerCase());
                break;
            ...
            case 'education':
                fieldValue = (candidate.education || [])
                    .map((e: any) => `${e.degree || ''} ${e.institution || ''}`.toLowerCase())
                    .join(' ');
                break;
            case 'source':
    ...
    private compareValues(fieldValue: any, operator: TagCondition['operator'], value: any): boolean {
        ...
        switch (operator) {
            ...
            case 'in':
                const inValues = Array.isArray(value) ? value.map(v => String(v).toLowerCase()) : [normalizedValue];
                if (Array.isArray(fieldValue)) {
            ...
            case 'not_in':
                const notInValues = Array.isArray(value) ? value.map(v => String(v).toLowerCase()) : [normalizedValue];
                if (Array.isArray(fieldValue)) {
        ...
        if (candidate.experience && Array.isArray(candidate.experience)) {
            ...
            for (const exp of candidate.experience) {
                if (exp.startDate) {
                    ...
                    const end = exp.endDate === 'Present' || !exp.endDate 
                        ? new Date() 
        ...
        const presets = [
            {
                ...
                tag: 'senior',
                conditions: [
                    { field: 'experience_years' as const, operator: 'greater_than' as const, value: 5 },
                ],
            },
            {
                ...
                tag: 'frontend',
                conditions: [
                    { field: 'skills' as const, operator: 'in' as const, value: ['react', 'vue', 'angular', 'javascript', 'typescript'] },
                ],
            },
            {
                ...
                tag: 'backend',
                conditions: [
                    { field: 'skills' as const, operator: 'in' as const, value: ['node', 'python', 'java', 'go', 'rust', 'c#'] },
                ],
            },
            {
                ...
                tag: 'fullstack',
                conditions: [
                    { field: 'title' as const, operator: 'contains' as const, value: 'full stack' },
                ],
            },
            {
                ...
                tag: 'devops',
                conditions: [
                    { field: 'skills' as const, operator: 'in' as const, value: ['docker', 'kubernetes', 'aws', 'terraform', 'jenkins', 'ci/cd'] },
                ],
            },
            {
                ...
                tag: 'data',
                conditions: [
                    { field: 'skills' as const, operator: 'in' as const, value: ['python', 'sql', 'machine learning', 'data science', 'pandas', 'tensorflow'] },
                ],
            },
            {
                ...
                tag: 'faang',
                conditions: [
                    { field: 'company' as const, operator: 'in' as const, value: ['google', 'meta', 'amazon', 'apple', 'netflix', 'microsoft', 'facebook'] },
                ],
            },
            {
                ...
                tag: 'remote',
                conditions: [
                    { field: 'location' as const, operator: 'contains' as const, value: 'remote' },
                ],
            },
        ];
