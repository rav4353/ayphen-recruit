...
export class DuplicateCheckDto {
    ...
    @ApiPropertyOptional({ description: 'Exclude this candidate ID from results' })
    @IsOptional()
    @IsString()
    excludeId?: string;
}
...
export class GdprConsentDto {
    ...
    @ApiPropertyOptional({ description: 'Has the candidate consented to marketing' })
    @IsOptional()
    marketingConsent?: boolean;

    @ApiPropertyOptional({ description: 'Source of consent (e.g., application form, email)' })
    @IsOptional()
    @IsString()
