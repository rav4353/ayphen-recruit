...
export class SemanticSearchService {
   ...
   */
  async updateCandidateEmbedding(
    candidateId: string,
    tenantId: string,
  ): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to update candidate embedding for ${candidateId}:`,
        error,
      );
    }
  }
  ...
  async findSimilarCandidates(
    ...
    const candidateText = this.buildCandidateText(
      candidate as CandidateWithEmbedding,
    );
  ...
  async matchCandidatesToJob(
    ...
    return this.searchCandidates({
      ...
      limit,
      minScore: 0.4,
      filters: (job as any).locations?.[0]?.city
        ? { location: (job as any).locations[0].city }
        : undefined,
    });
  }
  ...
  private generateMatchReason(
    ...
    if (matchedSkills.length > 0) {
      reasons.push(
        `Has ${matchedSkills.length} matching skill${matchedSkills.length > 1 ? "s" : ""}: ${matchedSkills.slice(0, 3).join(", ")}`,
      );
    }
    ...
    if (
      ...
      queryLower.includes(candidate.currentTitle.toLowerCase().split(" ")[0])
    ) {
  ...
  private async fallbackKeywordSearch(
    ...
    return candidates.map((candidate) => ({
      ...
      matchScore: this.keywordSimilarity(
        query,
        this.buildCandidateText(candidate),
      ),
