...
export class CollaborationService {
    ...
    async createComment(
        ...
        await this.prisma.activityLog.create({
            data: {
                ...
                userId,
                ...(dto.entityType === 'candidate' ? { candidateId: dto.entityId } : {}),
                ...(dto.entityType === 'application' ? { applicationId: dto.entityId } : {}),
                metadata,
            },
        });

        // Notify mentioned users
        await this.notifyMentions(tenantId, userId, mentions, dto.entityType, dto.entityId, dto.content);
    ...
    async deleteComment(tenantId: string, commentId: string, userId: string): Promise<void> {
        const log = await this.prisma.activityLog.findFirst({
            where: {
                action: 'COMMENT_CREATED',
                metadata: {
    ...
    async getNote(tenantId: string, noteId: string, userId: string): Promise<SharedNote> {
        const setting = await this.prisma.setting.findUnique({
            where: { tenantId_key: { tenantId, key: `${SHARED_NOTE_KEY}_${noteId}` } },
        });
    ...
    async getMentionSuggestions(tenantId: string, query: string): Promise<{
        id: string;
        name: string;
        email: string;
        avatar?: string;
    ...
    async getMyMentions(tenantId: string, userId: string): Promise<{
        commentId: string;
        entityType: string;
        entityId: string;
        mentionedBy: string;
