...
export class CommunicationEmailsService {
  ...
  async sendBulkEmail(
    userId: string,
    tenantId: string,
    data: { ids: string[]; subject: string; message: string },
  ) {
    ...
    for (const candidate of candidates) {
      try {
        ...
        this.logger.error(
          `Failed to send bulk email to candidate ${candidate.id}`,
          error,
        );
  ...
  async processInboundEmail(payload: {
    ...
    this.logger.log(
      `Processed inbound email from ${from} for candidate ${candidate.id}`,
    );
