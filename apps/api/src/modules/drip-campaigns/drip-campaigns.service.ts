...
export class DripCampaignsService {
    ...
    async getCampaign(tenantId: string, campaignId: string): Promise<DripCampaign> {
        const setting = await this.prisma.setting.findUnique({
            where: { tenantId_key: { tenantId, key: `${DRIP_CAMPAIGN_KEY}_${campaignId}` } },
        });
    ...
    async activateCampaign(tenantId: string, campaignId: string): Promise<DripCampaign> {
        const campaign = await this.getCampaign(tenantId, campaignId);
    ...
    async pauseCampaign(tenantId: string, campaignId: string): Promise<DripCampaign> {
        const campaign = await this.getCampaign(tenantId, campaignId);
    ...
    private async enrollRecipients(tenantId: string, campaign: DripCampaign): Promise<void> {
        ...
        nextStepAt.setHours(nextStepAt.getHours() + (firstStepDelay?.delayHours || 0));
        ...
        await this.prisma.setting.update({
            where: { tenantId_key: { tenantId, key: `${DRIP_CAMPAIGN_KEY}_${campaign.id}` } },
            data: { value: updatedCampaign as any },
        });
    }

    async getEnrollments(tenantId: string, campaignId: string): Promise<DripEnrollment[]> {
        const settings = await this.prisma.setting.findMany({
            where: {
                tenantId,
                key: { startsWith: `${DRIP_ENROLLMENT_KEY}_${campaignId}_` },
    ...
    async unsubscribeCandidate(tenantId: string, campaignId: string, candidateId: string): Promise<void> {
    ...
    private async processEnrollmentStep(tenantId: string, enrollment: DripEnrollment): Promise<void> {
        ...
        if (currentStep.trigger === 'CONDITION' && currentStep.condition) {
            ...
            if (previousStepCompletion) {
                const conditionMet = this.evaluateCondition(currentStep.condition, previousStepCompletion);
                if (!conditionMet) {
                    // Skip this step
        ...
        // Send email
        try {
            const personalizedSubject = this.personalizeContent(currentStep.subject, candidate);
            const personalizedBody = this.personalizeContent(currentStep.body, candidate);
            ...
            this.logger.log(`Drip email sent to ${candidate.email} for step ${currentStep.name}`);
        } catch (error) {
            this.logger.error(`Failed to send drip email to ${candidate.email}:`, error);
        }
    ...
    private moveToNextStep(enrollment: DripEnrollment, campaign: DripCampaign): void {
    ...
    private async updateEnrollment(tenantId: string, enrollment: DripEnrollment): Promise<void> {
    ...
    async getCampaignStats(tenantId: string, campaignId: string): Promise<{
        ...
        const stepStats = campaign.steps.map(step => {
            const stepsCompleted = enrollments.flatMap(e => e.stepsCompleted).filter(sc => sc.stepId === step.id);
            return {
                stepId: step.id,
                stepName: step.name,
