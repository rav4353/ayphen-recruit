...
export class EmailTemplatesService {
    ...
    getAvailableVariables(): TemplateVariable[] {
        return [
            // Candidate variables
            { name: '{{candidate.firstName}}', description: 'Candidate first name', example: 'John', category: 'candidate' },
            { name: '{{candidate.lastName}}', description: 'Candidate last name', example: 'Doe', category: 'candidate' },
            { name: '{{candidate.fullName}}', description: 'Candidate full name', example: 'John Doe', category: 'candidate' },
            { name: '{{candidate.email}}', description: 'Candidate email', example: 'john@example.com', category: 'candidate' },
            { name: '{{candidate.phone}}', description: 'Candidate phone', example: '+1234567890', category: 'candidate' },
            { name: '{{candidate.currentTitle}}', description: 'Current job title', example: 'Software Engineer', category: 'candidate' },
            { name: '{{candidate.currentCompany}}', description: 'Current company', example: 'Acme Inc', category: 'candidate' },
            
            // Job variables
            { name: '{{job.title}}', description: 'Job title', example: 'Senior Developer', category: 'job' },
            { name: '{{job.department}}', description: 'Department name', example: 'Engineering', category: 'job' },
            { name: '{{job.location}}', description: 'Job location', example: 'San Francisco, CA', category: 'job' },
            { name: '{{job.salaryRange}}', description: 'Salary range', example: '$120k - $150k', category: 'job' },
            
            // Company variables
            { name: '{{company.name}}', description: 'Company name', example: 'TechCorp', category: 'company' },
            { name: '{{company.website}}', description: 'Company website', example: 'https://techcorp.com', category: 'company' },
            
            // Interview variables
            { name: '{{interview.date}}', description: 'Interview date', example: 'Monday, January 15, 2024', category: 'interview' },
            { name: '{{interview.time}}', description: 'Interview time', example: '2:00 PM PST', category: 'interview' },
            { name: '{{interview.type}}', description: 'Interview type', example: 'Technical Interview', category: 'interview' },
            { name: '{{interview.duration}}', description: 'Interview duration', example: '60 minutes', category: 'interview' },
            { name: '{{interview.link}}', description: 'Meeting link', example: 'https://meet.google.com/abc-xyz', category: 'interview' },
            { name: '{{interview.interviewer}}', description: 'Interviewer name', example: 'Jane Smith', category: 'interview' },
            
            // User/Sender variables
            { name: '{{user.firstName}}', description: 'Sender first name', example: 'Jane', category: 'user' },
            { name: '{{user.lastName}}', description: 'Sender last name', example: 'Smith', category: 'user' },
            { name: '{{user.fullName}}', description: 'Sender full name', example: 'Jane Smith', category: 'user' },
            { name: '{{user.email}}', description: 'Sender email', example: 'jane@company.com', category: 'user' },
            { name: '{{user.title}}', description: 'Sender job title', example: 'HR Manager', category: 'user' },
        ];
    }
     ...
     */
    async getTemplate(tenantId: string, templateId: string): Promise<EmailTemplate> {
        const setting = await this.prisma.setting.findUnique({
            where: { tenantId_key: { tenantId, key: `${EMAIL_TEMPLATE_KEY}_${templateId}` } },
        });
     ...
     */
    async duplicateTemplate(tenantId: string, templateId: string, userId: string): Promise<EmailTemplate> {
     ...
     */
    async seedDefaultTemplates(tenantId: string, userId: string): Promise<EmailTemplate[]> {
...
<p>Best regards,<br>{{user.fullName}}<br>{{company.name}}</p>`,
        ...
        for (const tmpl of defaultTemplates) {
            try {
                ...
                this.logger.warn(`Failed to create default template ${tmpl.name}:`, error);
            }
        }
