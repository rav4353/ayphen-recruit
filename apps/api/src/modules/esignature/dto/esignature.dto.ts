import { IsString, IsOptional, IsEnum, IsArray, ValidateNested, IsBoolean, IsEmail } from 'class-validator';
...
export class ConfigureESignatureDto {
    @ApiProperty({ enum: ['DOCUSIGN', 'ADOBE_SIGN', 'ZOHO_SIGN'], default: 'DOCUSIGN' })
    ...
    @ApiPropertyOptional({ description: 'Base URL (e.g., https://demo.docusign.net for sandbox)' })
    @IsOptional()
    @IsString()
    baseUrl?: string;
}
