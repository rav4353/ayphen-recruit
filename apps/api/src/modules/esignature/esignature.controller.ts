...
export class ESignatureController {
  ...
  async configure(@Body() dto: ConfigureESignatureDto, @Req() req: any) {
    const tenantId = req.user.tenantId;
    const result = await this.esignatureService.configureSettings(
      tenantId,
      dto,
    );
  ...
  async getEnvelopeStatus(@Param("id") envelopeId: string, @Req() req: any) {
    const tenantId = req.user.tenantId;
    const status = await this.esignatureService.getEnvelopeStatus(
      tenantId,
      envelopeId,
    );
  ...
  async getEmbeddedSigningUrl(
    ...
    const tenantId = req.user.tenantId;
    const result = await this.esignatureService.getEmbeddedSigningUrl(
      tenantId,
      envelopeId,
      returnUrl,
    );
  ...
  async voidEnvelope(
    ...
    const tenantId = req.user.tenantId;
    const result = await this.esignatureService.voidEnvelope(
      tenantId,
      envelopeId,
      reason,
    );
