...
import {
  ConfigureESignatureDto,
  ConnectESignatureDto,
  SendForSignatureDto,
  ESignatureStatusResponse,
} from "./dto/esignature.dto";
...
export class ESignatureService {
  ...
  async getAuthUrl(tenantId: string): Promise<string> {
    const settings = await this.prisma.eSignatureSettings.findUnique({
      where: { tenantId },
    });
    if (!settings?.clientId || !settings?.clientSecret) {
      throw new BadRequestException(
        "DocuSign not configured. Please add credentials first.",
      );
    }
  ...
  async connect(tenantId: string, dto: ConnectESignatureDto) {
    ...
    const tokens = await this.docuSign.exchangeCodeForTokens(
      config,
      dto.code,
      dto.redirectUri,
    );
    const userInfo = await this.docuSign.getUserInfo(
      config,
      tokens.access_token,
    );
    ...
    if (!defaultAccount)
      throw new BadRequestException("No DocuSign account found");
  ...
  private async getValidAccessToken(settings: any): Promise<string> {
    if (
      ...
      new Date(settings.tokenExpiresAt) > new Date(Date.now() + 5 * 60 * 1000)
    ) {
      return settings.accessToken;
    }
    if (!settings.refreshToken)
      throw new BadRequestException(
        "E-signature connection expired. Please reconnect.",
      );
    ...
    const tokens = await this.docuSign.refreshAccessToken(
      config,
  ...
  async sendForSignature(tenantId: string, dto: SendForSignatureDto) {
    ...
    if (!settings?.isConfigured)
      throw new BadRequestException("E-signature is not configured");
    ...
    if (offer.status !== "APPROVED" && offer.status !== "SENT") {
      throw new BadRequestException(
        "Offer must be approved before sending for signature",
      );
    }
    ...
    const envelope = await this.docuSign.createEnvelope(
      ...
      settings.accountId!,
      {
        ...
        emailSubject:
          dto.emailSubject ||
          `Please sign your offer letter for ${offer.application.job.title}`,
  ...
  async getEnvelopeStatus(
    tenantId: string,
    envelopeId: string,
  ): Promise<ESignatureStatusResponse> {
  ...
  async getEmbeddedSigningUrl(
    ...
    const envelope = await this.prisma.eSignatureEnvelope.findFirst({
      where: { id: envelopeId, offer: { application: { job: { tenantId } } } },
      include: {
        offer: { include: { application: { include: { candidate: true } } } },
      },
    });
    ...
    if (!settings?.isConfigured)
      throw new BadRequestException("E-signature not configured");

    const accessToken = await this.getValidAccessToken(settings);
    const candidate = envelope.offer.application.candidate;

    const result = await this.docuSign.getEmbeddedSigningUrl(
      accessToken,
      settings.baseUrl!,
      settings.accountId!,
      envelope.externalId,
  ...
  async voidEnvelope(tenantId: string, envelopeId: string, reason: string) {
    ...
    const accessToken = await this.getValidAccessToken(settings);
    await this.docuSign.voidEnvelope(
      accessToken,
      settings.baseUrl!,
      settings.accountId!,
      envelope.externalId,
    ...
    await this.prisma.eSignatureEnvelope.update({
      where: { id: envelopeId },
      data: { status: "VOIDED" },
    });
    await this.prisma.offer.update({
  ...
  async handleWebhook(payload: any) {
    ...
    const status = this.mapDocuSignStatus(
