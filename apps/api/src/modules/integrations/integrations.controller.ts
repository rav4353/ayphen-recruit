...
import { WebhookManagementService, WebhookEvent } from './webhook-management.service';
...
export class JobBoardsController {
  ...
  configure(
    ...
    @Body() body: { provider: JobBoardProvider; apiKey: string; apiSecret?: string; companyId?: string },
  ) {
...
export class IndeedFeedController {
  ...
  configure(
    @CurrentUser() user: JwtPayload,
    @Body() body: { publisherId: string; companyName: string; includeDescription?: boolean; includeSalary?: boolean },
  ) {
...
export class ZipRecruiterController {
  ...
  configure(
    @CurrentUser() user: JwtPayload,
    @Body() body: { apiKey: string; publisherId: string; sandboxMode?: boolean },
  ) {
    return this.zipService.configure(user.tenantId, { ...body, sandboxMode: body.sandboxMode ?? true });
  }
...
export class HRISController {
  ...
  @RequirePermissions(Permission.SETTINGS_VIEW)
  getSyncHistory(@CurrentUser() user: JwtPayload, @Query('limit') limit?: string) {
    return this.hrisService.getSyncHistory(user.tenantId, limit ? parseInt(limit) : 10);
  }
}
...
export class MessagingController {
  ...
  configureTeams(
    @CurrentUser() user: JwtPayload,
    @Body() body: { webhookUrl: string; notificationChannels?: Record<string, string> },
  ) {
...
export class WebhooksController {
  ...
  createWebhook(
    @CurrentUser() user: JwtPayload,
    @Body() body: { name: string; url: string; events: WebhookEvent[]; headers?: Record<string, string> },
  ) {
  ...
  updateWebhook(
    @CurrentUser() user: JwtPayload,
    @Param('id') id: string,
    @Body() body: { name?: string; url?: string; events?: WebhookEvent[]; isActive?: boolean },
  ) {
  ...
  getDeliveries(
    ...
    return this.webhookService.getDeliveryHistory(user.tenantId, id, limit ? parseInt(limit) : 50);
  }
}
