...
export class JobBoardsService {
  ...
  async getSettings(
    tenantId: string,
  ): Promise<{ [key: string]: { isConfigured: boolean; companyId?: string } }> {
    const setting = await this.prisma.setting.findUnique({
      where: { tenantId_key: { tenantId, key: JOB_BOARD_SETTINGS_KEY } },
    });

    const configs =
      (setting?.value as unknown as Record<string, JobBoardConfig>) || {};
  ...
  async getProviderSettings(
    tenantId: string,
    provider: JobBoardProvider,
  ): Promise<{ isConfigured: boolean; companyId?: string }> {
  ...
  async postJob(
  ...
  ): Promise<{ provider: string; status: string; url?: string }[]> {
    ...
    const targetProviders =
      providers || (Object.keys(configs) as JobBoardProvider[]);
    ...
    const response = await fetch("https://apis.indeed.com/v2/jobs", {
      ...
      body: JSON.stringify({
        ...
        salary: job.salaryMax
          ? { max: Number(job.salaryMax), currency: "USD" }
          : undefined,
      }),
    });
  ...
  async postToZipRecruiter(job: Job, config?: JobBoardConfig): Promise<string> {
    ...
    return (
      result.url || `https://www.ziprecruiter.com/jobs/${result.id || job.id}`
    );
  }
  ...
  async postToGlassdoor(job: Job, config?: JobBoardConfig): Promise<string> {
    ...
    if (!config?.apiKey || !config?.companyId) {
      throw new BadRequestException(
        "Glassdoor API key and company ID required",
      );
    }
