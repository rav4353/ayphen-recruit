...
interface NotificationPayload {
  type: 'NEW_APPLICATION' | 'INTERVIEW_SCHEDULED' | 'OFFER_ACCEPTED' | 'OFFER_DECLINED' | 'CANDIDATE_HIRED' | 'CUSTOM';
  title: string;
  message: string;
  url?: string;
...
export class SlackTeamsService {
   ...
   */
  async getSlackConfig(tenantId: string): Promise<{ isConfigured: boolean; channels?: string[] }> {
    ...
    return {
      ...
      channels: config?.notificationChannels ? Object.keys(config.notificationChannels) : [],
    };
  }
   ...
   */
  async sendSlackNotification(tenantId: string, payload: NotificationPayload, channelOverride?: string): Promise<{ success: boolean; messageId?: string }> {
    const setting = await this.prisma.setting.findUnique({
      where: { tenantId_key: { tenantId, key: SLACK_SETTINGS_KEY } },
    });
    ...
    const channel = channelOverride || this.getSlackChannelForType(config, payload.type);
    if (!channel) {
      throw new BadRequestException('No channel configured for this notification type');
    }
    ...
    try {
      ...
      messageId = result.ts;
      this.logger.log(`Slack notification sent successfully to ${channel}: ${payload.title}`);
    } catch (error: any) {
      this.logger.error(`Failed to send Slack notification: ${error.message}`);
      throw new BadRequestException(`Failed to send Slack notification: ${error.message}`);
    }
   ...
   * Get Slack channels
   */
  async getSlackChannels(tenantId: string): Promise<{ id: string; name: string }[]> {
    ...
    try {
      ...
      const result = await response.json() as { ok: boolean; channels?: { id: string; name: string }[]; error?: string };
   ...
   */
  async testSlackConnection(tenantId: string): Promise<{ success: boolean; message: string; workspace?: string }> {
    try {
      const config = await this.getSlackConfigOrThrow(tenantId);
      ...
      this.logger.log(`Slack connection verified for workspace: ${result.team}`);
      return { success: true, message: `Connected to Slack workspace: ${result.team}`, workspace: result.team };
    } catch (error: any) {
   ...
   */
  async getTeamsConfig(tenantId: string): Promise<{ isConfigured: boolean; hasWebhook?: boolean }> {
    const setting = await this.prisma.setting.findUnique({
   ...
   */
  async sendTeamsNotification(tenantId: string, payload: NotificationPayload): Promise<{ success: boolean }> {
    const setting = await this.prisma.setting.findUnique({
      where: { tenantId_key: { tenantId, key: TEAMS_SETTINGS_KEY } },
    });
    ...
    try {
      ...
      throw new BadRequestException(`Failed to send Teams notification: ${error.message}`);
    }
   ...
   */
  async testTeamsConnection(tenantId: string): Promise<{ success: boolean; message: string }> {
    try {
      const setting = await this.prisma.setting.findUnique({
        where: { tenantId_key: { tenantId, key: TEAMS_SETTINGS_KEY } },
      });
      ...
      if (!response.ok) {
        return { success: false, message: `Teams webhook returned status ${response.status}` };
      }

      this.logger.log('Teams connection test successful');
      return { success: true, message: 'Teams webhook verified - test message sent' };
    } catch (error: any) {
      return { success: false, message: error.message };
    }
  }
   ...
   */
  async sendNotification(tenantId: string, payload: NotificationPayload): Promise<{ slack?: boolean; teams?: boolean }> {
  ...
  async notifyNewApplication(tenantId: string, application: any) {
    ...
    return this.sendNotification(tenantId, {
      ...
      fields: [
        { name: 'Candidate', value: `${application.candidate?.firstName} ${application.candidate?.lastName}` },
  ...
  async notifyInterviewScheduled(tenantId: string, interview: any) {
    ...
    return this.sendNotification(tenantId, {
      ...
      fields: [
        { name: 'Candidate', value: `${interview.candidate?.firstName} ${interview.candidate?.lastName}` },
        { name: 'Type', value: interview.type },
        { name: 'Date', value: new Date(interview.scheduledAt).toLocaleString() },
        { name: 'Interviewer', value: interview.interviewer?.firstName || 'TBD' },
      ],
      color: '#2196F3',
    });
  }
  ...
  async notifyOfferAccepted(tenantId: string, offer: any) {
    ...
    return this.sendNotification(tenantId, {
      ...
      fields: [
        { name: 'Candidate', value: `${offer.candidate?.firstName} ${offer.candidate?.lastName}` },
        { name: 'Position', value: offer.job?.title || 'Unknown' },
        { name: 'Start Date', value: offer.startDate ? new Date(offer.startDate).toLocaleDateString() : 'TBD' },
      ],
      color: '#4CAF50',
    });
  }
  ...
  private getSlackChannelForType(config: SlackConfig, type: NotificationPayload['type']): string | undefined {
    switch (type) {
      case 'NEW_APPLICATION':
        return config.notificationChannels?.newApplication || config.defaultChannelId;
      case 'INTERVIEW_SCHEDULED':
        return config.notificationChannels?.interviewScheduled || config.defaultChannelId;
      case 'OFFER_ACCEPTED':
      case 'CANDIDATE_HIRED':
        return config.notificationChannels?.offerAccepted || config.defaultChannelId;
      default:
  ...
  private buildTeamsCard(payload: NotificationPayload): any {
    const facts = payload.fields?.map(f => ({ name: f.name, value: f.value })) || [];
