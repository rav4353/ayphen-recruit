...
export interface FeedbackAnalytics {
    ...
    recommendationBreakdown: { recommendation: string; count: number; percentage: number }[];
...
export class InterviewAnalyticsService {
    ...
    async getFeedbackAnalytics(
        ...
        const recommendationBreakdown = this.calculateRecommendationBreakdown(feedbacks);

        // Interviewer performance
        const interviewerPerformance = this.calculateInterviewerPerformance(interviews);
    ...
    async getInterviewerAnalytics(tenantId: string, interviewerId: string) {
        const analytics = await this.getFeedbackAnalytics(tenantId, { interviewerId });
        ...
        const interviewerInsights = await this.getInterviewerInsights(interviewerId);
    ...
    async getHiringFunnelAnalytics(tenantId: string, jobId?: string) {
        ...
        const stages = {
            ...
            offered: applications.filter(a => (a.status as string) === 'OFFERED').length,
            hired: applications.filter(a => (a.status as string) === 'HIRED').length,
        };

        const conversionRates = {
            screenToInterview: stages.screened > 0 
                ? Math.round((stages.interviewed / stages.screened) * 100) : 0,
            interviewToPositive: stages.interviewed > 0 
                ? Math.round((stages.positiveRecommendation / stages.interviewed) * 100) : 0,
            positiveToOffer: stages.positiveRecommendation > 0 
                ? Math.round((stages.offered / stages.positiveRecommendation) * 100) : 0,
            offerToHire: stages.offered > 0 
                ? Math.round((stages.hired / stages.offered) * 100) : 0,
            overallConversion: stages.applied > 0 
                ? Math.round((stages.hired / stages.applied) * 100) : 0,
        };
    ...
    async getFeedbackQualityMetrics(tenantId: string) {
        ...
        for (const m of qualityMetrics) {
            if (!byInterviewer.has(m.interviewerId)) {
                byInterviewer.set(m.interviewerId, { name: m.interviewerName, scores: [] });
            }
            byInterviewer.get(m.interviewerId)!.scores.push(m.qualityScore);
        }

        const interviewerQuality = Array.from(byInterviewer.entries()).map(([id, data]) => ({
            interviewerId: id,
            interviewerName: data.name,
            averageQuality: Math.round(data.scores.reduce((a, b) => a + b, 0) / data.scores.length),
            feedbackCount: data.scores.length,
        })).sort((a, b) => b.averageQuality - a.averageQuality);

        return {
            overallAverageQuality: qualityMetrics.length > 0
                ? Math.round(qualityMetrics.reduce((a, b) => a + b.qualityScore, 0) / qualityMetrics.length)
                : 0,
    ...
    private calculateOverview(interviews: any[], feedbacks: any[]) {
        ...
        const interviewsWithFeedback = interviews.filter(i => i.feedbacks.length > 0);
        ...
        return {
            ...
            averageRating: ratedFeedbacks.length > 0 
                ? Math.round((totalRating / ratedFeedbacks.length) * 10) / 10 
                : 0,
            completionRate: completedInterviews.length > 0
                ? Math.round((interviewsWithFeedback.length / completedInterviews.length) * 100)
                : 0,
        };
    }
    ...
    private calculateRecommendationBreakdown(feedbacks: any[]) {
        ...
        const recommendationLabels = ['STRONG_HIRE', 'HIRE', 'NO_DECISION', 'NO_HIRE', 'STRONG_NO_HIRE'];
        ...
        for (const f of feedbacksWithRec) {
            recommendations.set(f.recommendation, (recommendations.get(f.recommendation) || 0) + 1);
        }

        const total = feedbacksWithRec.length || 1;
        return Array.from(recommendations.entries()).map(([recommendation, count]) => ({
            recommendation,
            count,
            percentage: Math.round((count / total) * 100),
        }));
    ...
    private calculateInterviewerPerformance(interviews: any[]) {
        const byInterviewer = new Map<string, {
            name: string;
            interviews: number;
            feedbacks: any[];
        }>();
        ...
        return Array.from(byInterviewer.entries()).map(([id, data]) => {
    ...
    private calculateTimeAnalysis(interviews: any[], feedbacks: any[]) {
        ...
        for (const period of periods) byPeriod.set(period, { ratings: [], count: 0 });
    ...
    private extractSkillsAnalysis(feedbacks: any[]) {
        ...
        const commonSkills = [
            'communication', 'problem solving', 'technical skills', 'teamwork',
            'leadership', 'adaptability', 'creativity', 'attention to detail',
            'time management', 'critical thinking', 'coding', 'system design',
    ...
    private async getInterviewerInsights(interviewerId: string) {
        ...
        const avgRating = recentFeedbacks.reduce((sum, f) => sum + (f.rating || 0), 0) / (recentFeedbacks.length || 1);
        
        if (avgRating > 4) {
            insights.push('Consistently provides high ratings - may want to calibrate expectations');
        } else if (avgRating < 2.5) {
            insights.push('Tends to provide lower ratings - consider calibration session');
        }

        // Check feedback completeness
        const incompleteFeedbacks = recentFeedbacks.filter(f => 
            !f.strengths || !f.weaknesses || !f.recommendation
        ).length;

        if (incompleteFeedbacks > recentFeedbacks.length * 0.3) {
            insights.push('Often submits incomplete feedback - encourage detailed notes');
        }
