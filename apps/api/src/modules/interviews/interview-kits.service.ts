...
export class InterviewKitsService {
   ...
   */
  async update(kitId: string, dto: Partial<CreateInterviewKitDto>, tenantId: string, userId: string) {
    ...
    const scorecard = dto.scorecard !== undefined
      ? dto.scorecard ? { ...dto.scorecard, id: this.generateId('sc') } : null
   ...
   */
  async duplicate(kitId: string, newName: string, tenantId: string, userId: string) {
    const existing = await this.findById(kitId, tenantId);
   ...
   */
  async assignToInterview(kitId: string, interviewId: string, tenantId: string, userId: string) {
