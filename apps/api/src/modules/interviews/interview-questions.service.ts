...
export class InterviewQuestionsService {
  ...
  async findAll(
    tenantId: string,
    filters?: { category?: string; difficulty?: string; skills?: string },
  ) {
    const logs = await this.prisma.activityLog.findMany({
      where: {
        action: {
          in: ["INTERVIEW_QUESTION_CREATED", "INTERVIEW_QUESTION_UPDATED"],
        },
        metadata: {
          path: ["tenantId"],
          equals: tenantId,
  ...
  async update(
    ...
    const updatedMetadata = {
      ...
      timeMinutes:
        dto.timeMinutes !== undefined ? dto.timeMinutes : existing.timeMinutes,
  ...
  async generateQuestionsForJob(
    ...
    const templates = [
      {
        q: `Tell me about your experience with ${skills[0] || "this role"}`,
        cat: "Experience",
        diff: "EASY" as const,
      },
      {
        q: `How would you approach a challenging ${job.title} project?`,
        cat: "Problem Solving",
      ...
      {
        q: `What's your approach to code review and collaboration?`,
        cat: "Teamwork",
        diff: "EASY" as const,
      },
      {
      ...
      {
        q: `Explain a complex technical concept to a non-technical stakeholder`,
        cat: "Communication",
        diff: "HARD" as const,
      },
      {
      ...
      const question = await this.create(
        {
          ...
          timeMinutes:
            template.diff === "HARD" ? 10 : template.diff === "MEDIUM" ? 7 : 5,
        },
        tenantId,
        userId,
      );
