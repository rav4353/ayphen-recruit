...
export class InterviewReminderService {
  ...
  private async sendReminders(interview: any) {
    ...
    try {
      ...
      this.logger.error(
        `Failed to send email reminder to candidate ${candidate.email}`,
        e,
      );
    }
    ...
    if (smsEnabled && candidate.phone) {
      try {
        ...
        if (result.success) {
          ...
          this.logger.warn(
            `SMS reminder failed for candidate ${candidate.phone}: ${result.error}`,
          );
        }
      } catch (e) {
        this.logger.error(
          `Failed to send SMS reminder to candidate ${candidate.phone}`,
          e,
        );
      }
    }
    ...
    try {
      ...
      this.logger.log(
        `Email reminder sent to interviewer ${interviewer.email}`,
      );
    } catch (e) {
      this.logger.error(
        `Failed to send email reminder to interviewer ${interviewer.email}`,
        e,
      );
    }
    ...
    if (smsEnabled && interviewer.phone) {
      try {
        ...
        if (result.success) {
          this.logger.log(
            `SMS reminder sent to interviewer ${interviewer.phone}`,
          );
        } else {
          this.logger.warn(
            `SMS reminder failed for interviewer ${interviewer.phone}: ${result.error}`,
          );
        }
      } catch (e) {
        this.logger.error(
          `Failed to send SMS reminder to interviewer ${interviewer.phone}`,
          e,
        );
      }
    }
  ...
  private async isSmsReminderEnabled(tenantId: string): Promise<boolean> {
    try {
      ...
      const setting = await this.prisma.setting.findUnique({
        where: {
          tenantId_key: { tenantId, key: "interview_sms_reminders_enabled" },
        },
      });
   ...
   */
  async sendImmediateSmsReminder(
    interviewId: string,
