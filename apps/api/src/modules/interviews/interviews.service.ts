...
export class InterviewsService {
    ...
    async create(dto: CreateInterviewDto, tenantId: string, createdByUserId?: string) {
        ...
        if (dto.syncToCalendar) {
            try {
                await this.syncInterviewToCalendar(interview, interviewer, application, tenantId);
            } catch (error) {
    ...
    async bookSelfScheduleSlot(
        ...
        const scheduledAt = new Date(`${selectedSlot.date}T${selectedSlot.startTime}`);
        ...
        try {
            ...
            console.error('Failed to sync self-scheduled interview to calendar:', error);
        }
    ...
    private async getCommonAvailability(
        ...
        for (let i = 1; i < allInterviewerSlots.length; i++) {
            commonSlots = commonSlots.filter(slot1 =>
                allInterviewerSlots[i].some(slot2 =>
    ...
    async getInterviewPanels(jobId: string, tenantId: string) {
        ...
        const allInterviewerIds = Array.from(panelMap.values())
        ...
        return Array.from(panelMap.values()).map(panel => ({
            ...
            interviewers: (panel.interviewerIds || []).map((id: string) => interviewerMap.get(id)).filter(Boolean),
        }));
    }
     ...
     */
    async deleteInterviewPanel(panelId: string, tenantId: string, userId: string) {
        const panelLog = await this.prisma.activityLog.findFirst({
            where: {
                action: 'INTERVIEW_PANEL_CREATED',
                metadata: {
    ...
    async getAvailableSlots(
            ...
            if (calendarConnection) {
                ...
                if (interviewer) {
                    ...
                    if (Array.isArray(freeBusy)) {
                        calendarBusySlots = freeBusy.flatMap(fb => 
                            fb.slots?.filter((slot: any) => !slot.available).map((slot: any) => ({ 
        ...
        while (currentTime + slotDuration <= endOfDay.getTime()) {
            ...
            const hasInterviewConflict = existingInterviews.some(interview => {
                ...
                const interviewEnd = interviewStart + (interview.duration || 60) * 60000;
                return (
                    (currentTime >= interviewStart && currentTime < interviewEnd) ||
                    (currentTime + slotDuration > interviewStart && currentTime + slotDuration <= interviewEnd)
                );
            });
            ...
            const hasCalendarConflict = calendarBusySlots.some(busy => {
                ...
                return (
                    ...
                    (currentTime + slotDuration > busyStart && currentTime + slotDuration <= busyEnd)
                );
            });
    ...
    async getFeedbackSummary(applicationId: string, tenantId: string) {
        ...
        const categoryAverages = Object.entries(scoreCategories).map(([category, scores]) => ({
            category,
        ...
        if (recCounts.length > 0) {
            const hireCount = (recommendations['STRONG_HIRE'] || 0) + (recommendations['HIRE'] || 0);
            const noHireCount = (recommendations['STRONG_NO_HIRE'] || 0) + (recommendations['NO_HIRE'] || 0);

            if (hireCount > noHireCount && hireCount >= allFeedback.length / 2) {
                overallRecommendation = 'HIRE';
            } else if (noHireCount > hireCount && noHireCount >= allFeedback.length / 2) {
        ...
        return {
            ...
            pendingFeedback: interviews.filter(i => i.status === 'COMPLETED' && i.feedbacks.length === 0).length,
            summary: {
     ...
     */
    async getFeedbackComparison(jobId: string, tenantId: string, candidateIds?: string[]) {
        const job = await this.prisma.job.findFirst({
            where: { id: jobId, tenantId },
        });
    ...
    async getPendingFeedbackRequests(userId: string, tenantId: string) {
        ...
        return interviews.map(i => ({
            ...
            daysSinceInterview: Math.floor(
                (Date.now() - new Date(i.scheduledAt).getTime()) / (1000 * 60 * 60 * 24)
            ),
        }));
    }
}
