...
export class JobPostingAnalyticsService {
     ...
     */
    async getJobAnalytics(tenantId: string, jobId: string): Promise<JobPostingMetrics> {
        ...
        const uniqueSessions = new Set(viewEvents.map(v => {
        ...
        for (const app of applications) {
            ...
            if (firstView) {
                const timeDiff = new Date(appDate).getTime() - new Date(firstView.createdAt).getTime();
                totalTimeToApply += timeDiff / (1000 * 60 * 60); // Convert to hours
                applyCount++;
            }
        }
        const averageTimeToApply = applyCount > 0 ? totalTimeToApply / applyCount : 0;
        ...
        const sourceBreakdown = this.calculateSourceBreakdown(viewEvents, applicationsWithSource);

        // Daily metrics (last 30 days)
        const dailyMetrics = this.calculateDailyMetrics(viewEvents, applicationsWithSource);
    ...
    async getAllJobsAnalytics(tenantId: string): Promise<{
        ...
        const activeJobsCount = jobs.filter(j => (j.status as string) === 'PUBLISHED').length;
        ...
        for (const job of jobs) {
            ...
            const uniqueSessions = new Set(jobViews.map(v => {
                const metadata = v.metadata as unknown as JobViewMetadata | null;
                return metadata?.sessionId || v.id;
            }));
            ...
            for (const app of jobApplications) {
                const firstView = jobViews.find(v => {
                    const metadata = v.metadata as unknown as JobViewMetadata | null;
                    return metadata?.sessionId && new Date(v.createdAt) < new Date(app.appliedAt);
                });
                if (firstView) {
                    const timeDiff = new Date(app.appliedAt).getTime() - new Date(firstView.createdAt).getTime();
            ...
            const sourceBreakdown = this.calculateSourceBreakdown(jobViews, applicationsWithSource);
            const dailyMetrics = this.calculateDailyMetrics(jobViews, applicationsWithSource);
        ...
        const avgViewToApplyRate = totalViews > 0 ? (totalApplications / totalViews) * 100 : 0;
     ...
     */
    async getJobBoardPerformance(tenantId: string): Promise<JobBoardPerformance[]> {
        const applications = await this.prisma.application.findMany({
            where: { job: { tenantId } },
            select: {
                id: true,
        ...
        for (const view of viewEvents) {
            ...
            if (!boardStats.has(source)) {
                boardStats.set(source, { views: 0, applications: [], jobIds: new Set() });
            }
        ...
        for (const [board, stats] of boardStats.entries()) {
            const conversionRate = stats.views > 0 
                ? (stats.applications.length / stats.views) * 100 
                : 0;
        ...
        return performance.sort((a, b) => b.totalApplications - a.totalApplications);
    }

    /**
     * Get source attribution report
     */
    async getSourceAttributionReport(tenantId: string, dateRange?: { start: Date; end: Date }): Promise<{
        sources: {
            source: string;
            candidates: number;
            applications: number;
        ...
        const sources = Array.from(sourceStats.entries()).map(([source, stats]) => ({
            ...
            hires: stats.hires,
            hiringRate: stats.applications > 0 
                ? Math.round((stats.hires / stats.applications) * 10000) / 100 
                : 0,
            avgDaysToHire: stats.hireDays.length > 0
                ? Math.round(stats.hireDays.reduce((a, b) => a + b, 0) / stats.hireDays.length)
                : 0,
        }));
        ...
        return {
            ...
            totalHires: applications.filter(a => (a.status as string) === 'HIRED').length,
        };
    }
    ...
    private calculateSourceBreakdown(
        ...
        const sourceStats = new Map<string, { views: number; applications: ApplicationWithSource[] }>();
        ...
        return Array.from(sourceStats.entries()).map(([source, stats]) => {
            ...
            const progressedApps = stats.applications.filter(a => 
                !['NEW', 'REJECTED', 'WITHDRAWN'].includes(a.status)
            ).length;
            const qualityScore = stats.applications.length > 0
    ...
    private calculateDailyMetrics(
        ...
        const last30Days = new Map<string, { views: number; applications: number }>();
