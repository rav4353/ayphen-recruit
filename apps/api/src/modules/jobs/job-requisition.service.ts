...
export class JobRequisitionService {
  ...
  async createRequisition(
    ...
    const approverIds = dto.approverIds || config.defaultApprovers || [];

    // Add finance approver if salary exceeds threshold
    if (
      config.requireFinanceApproval &&
      dto.salaryMax &&
      dto.salaryMax > config.salaryThresholdForFinance
    ) {
    ...
    const approvalChain: ApprovalStep[] = approverIds.map(
      (approverId, index) => {
  ...
  async getRequisitions(
    ...
    let requisitions = settings.map(
      (s) => s.value as unknown as JobRequisition,
    );
    ...
    if (filters?.requestedBy) {
      requisitions = requisitions.filter(
        (r) => r.requestedBy === filters.requestedBy,
      );
    }

    return requisitions.sort(
      (a, b) =>
  ...
  async getRequisition(
    ...
    const setting = await this.prisma.setting.findUnique({
      where: {
        tenantId_key: { tenantId, key: `${REQUISITION_KEY}_${requisitionId}` },
      },
    });
  ...
  async submitForApproval(
    ...
    const updated = await this.updateRequisitionStatus(
      tenantId,
      requisitionId,
      {
        status: "PENDING_APPROVAL",
  ...
  async cancelRequisition(
    ...
    if (requisition.requestedBy !== userId) {
      throw new BadRequestException(
        "Only the requester can cancel this requisition",
      );
    }
