...
export class JobsController {
  ...
  async findAll(
    ...
    const enrichedJobs = await this.jobsService.enrichJobsWithStatusColors(
      result.jobs,
      user.tenantId,
    );
  ...
  async findOne(
    ...
    const enrichedJob = await this.jobsService.enrichJobsWithStatusColors(
      [job],
      job.tenantId,
    );
  ...
  async update(
    ...
    this.validateTenantAccess(user.tenantId, tenantId);
    const job = await this.jobsService.update(id, dto, user.sub);
    // Check if some edits are pending approval
    if ((job as any)._pendingEdits) {
      return ApiResponse.success(
        job,
        "Some changes require approval and have been submitted for review",
      );
    }
  ...
  async submitApproval(
    ...
    this.validateTenantAccess(user.tenantId, tenantId);
    const job = await this.jobsService.submitForApproval(
      id,
      dto.approverIds || [],
      user.sub,
      dto.comment,
  ...
  @Post(":id/approve")
  @UseGuards(JwtAuthGuard)
  @ApiOperation({ summary: "Approve or reject job" })
  async approveJob(
    @Param("tenantId") tenantId: string,
    @Param("id") id: string,
    ...
    this.validateTenantAccess(user.tenantId, tenantId);
    const job = await this.jobsService.approve(
      id,
      user.sub,
      dto.status,
      dto.comment,
    ...
    return ApiResponse.success(job, message);
  }
  ...
  async getRequisitionStats(
    ...
    return ApiResponse.success(
      stats,
      "Requisition stats retrieved successfully",
    );
  }
  ...
  async convertRequisitionToJob(
    ...
    return ApiResponse.created(
      result,
      "Requisition converted to job successfully",
    );
  }
}
