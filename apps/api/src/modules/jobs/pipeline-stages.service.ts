...
export class PipelineStagesService {
  ...
  async getPipeline(tenantId: string, pipelineId: string): Promise<Pipeline> {
    const setting = await this.prisma.setting.findUnique({
      where: {
        tenantId_key: { tenantId, key: `${PIPELINE_KEY}_${pipelineId}` },
      },
    });
    ...
    for (const pipeline of pipelines) {
      if (pipeline.isDefault !== (pipeline.id === pipelineId)) {
        ...
        await this.prisma.setting.update({
          where: {
            tenantId_key: { tenantId, key: `${PIPELINE_KEY}_${pipeline.id}` },
          },
  ...
  async addStage(
    ...
    return this.updatePipeline(tenantId, pipelineId, {
  ...
  async createPresetPipelines(tenantId: string): Promise<Pipeline[]> {
    ...
    for (const preset of presets) {
      try {
        ...
        this.logger.warn(
          `Failed to create preset pipeline ${preset.name}:`,
          error,
        );
      }
    }
