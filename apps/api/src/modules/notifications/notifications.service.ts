...
export class NotificationsService {
  ...
  async findAllForUser(userId: string, filters?: NotificationFilters) {
    ...
    if (filters?.allowedTypes?.length) {
      where.type = where.type ? where.type : { in: filters.allowedTypes };
    }
  ...
  private checkPreferences(preferences: any, type: NotificationType): boolean {
    switch (type) {
      ...
      case "INTERVIEW":
        return (
          preferences.interviewScheduled ||
          preferences.interviewReminder ||
          preferences.interviewFeedback
        );
      case "OFFER":
        return (
          preferences.offerCreated ||
          preferences.offerApproval ||
          preferences.offerAccepted ||
          preferences.offerDeclined
        );
      case "JOB":
  ...
  /**
   * Notify candidate about interview invitation with acceptance link
   */
  async notifyCandidateInterviewInvitation(
    interview: any,
    confirmUrl: string,
    tenantId: string,
  ) {
    ...
    return this.emailService.sendInterviewInvitationEmail(
