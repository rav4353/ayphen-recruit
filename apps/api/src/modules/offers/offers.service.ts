...
export class OffersService {
  ...
  async submit(tenantId: string, id: string) {
    ...
    try {
      ...
      if (workflows && workflows.length > 0) {
        ...
        const offerWorkflow =
          workflows.find((w) => w.name.includes("Offer")) || workflows[0];

        if (offerWorkflow && offerWorkflow.steps?.length > 0) {
          console.log(
            `[OffersService] Using approval workflow: ${offerWorkflow.name}`,
          );
          finalApproverIds = offerWorkflow.steps
            .map((step: any) => {
  ...
  async acceptOffer(token: string, signature: string) {
    ...
    if (offer.status !== OfferStatus.SENT) {
      throw new BadRequestException(
        "Offer is not in a valid state to be accepted",
      );
    }
  ...
  async declineOffer(token: string, reason: string) {
    ...
    if (offer.status !== OfferStatus.SENT) {
      throw new BadRequestException(
        "Offer is not in a valid state to be declined",
      );
    }
  ...
  async generateOfferLetter(tenantId: string, offerId: string) {
    ...
    const variables: Record<string, string> = {
      ...
      "{{employment_type}}":
        offer.application.job.employmentType || "Full-time",
      ...
      "{{bonus}}": offer.bonus
        ? this.formatCurrency(Number(offer.bonus), offer.currency)
        : "N/A",
      ...
      "{{expiry_date}}": offer.expiresAt
        ? this.formatDate(offer.expiresAt)
        : "N/A",
  ...
  async previewOfferLetter(
    ...
    const variables: Record<string, string> = {
      ...
      "{{salary}}": data.salary
        ? this.formatCurrency(data.salary, data.currency || "USD")
        : "[SALARY]",
      "{{bonus}}": data.bonus
        ? this.formatCurrency(data.bonus, data.currency || "USD")
        : "N/A",
      "{{equity}}": data.equity || "N/A",
      "{{start_date}}": data.startDate
        ? this.formatDate(new Date(data.startDate))
        : "[START DATE]",
      "{{offer_date}}": this.formatDate(new Date()),
...
<div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 40px;">
  ...
  async submitCounterOffer(
    ...
    const recipientIds = [];
    if (offer.application.job.recruiterId)
      recipientIds.push(offer.application.job.recruiterId);
    if (offer.application.job.hiringManagerId)
      recipientIds.push(offer.application.job.hiringManagerId);
    ...
    if (data.action === "ACCEPT") {
      ...
      if (counterMetadata.counterOffer.requestedStartDate) {
        updateData.startDate = new Date(
          counterMetadata.counterOffer.requestedStartDate,
        );
      }
  ...
  async getNegotiationHistory(offerId: string, tenantId: string) {
    ...
    return {
      ...
      negotiationCount: activities.filter(
        (a) =>
          a.action === "OFFER_COUNTER_SUBMITTED" ||
          a.action === "OFFER_REVISED",
      ).length,
  ...
  async getOfferComparison(offerId: string, tenantId: string) {
    ...
    const originalTerms = (originalActivity?.metadata as any)
      ?.originalTerms || {
      salary: offer.salary,
      bonus: offer.bonus,
      equity: offer.equity,
    ...
    if (originalTerms.salary !== currentTerms.salary) {
      ...
      changes.push({
        ...
        difference: diff,
        percentChange: originalTerms.salary
          ? ((diff / Number(originalTerms.salary)) * 100).toFixed(1)
          : null,
      });
    }
    ...
    if (
      originalTerms.startDate?.toString() !== currentTerms.startDate?.toString()
    ) {
      changes.push({
        field: "startDate",
