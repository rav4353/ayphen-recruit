...
export class CustomReportsService {
  ...
  getAvailableColumns(entityType: string): ReportColumn[] {
    const columns: Record<string, ReportColumn[]> = {
      ...
      interviews: [
        {
          id: "candidateName",
          field: "application.candidate.firstName",
          label: "Candidate Name",
          type: "string",
          filterable: true,
        },
  ...
  async executeReport(
    ...
    const { entityType, columns, filters, sorts, groupBy, dateRange, limit } =
      definition;
  ...
  private resolveDateRange(dateRange: ReportDefinition["dateRange"]): {
    start?: string;
    end?: string;
  } {
    if (!dateRange) return {};
  ...
  async exportToCSV(
    ...
    const headers = definition.columns
      .map(
        (colId) => availableColumns.find((c) => c.id === colId)?.label || colId,
      )
      .join(",");
