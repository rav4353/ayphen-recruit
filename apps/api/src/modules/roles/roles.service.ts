...
export class RolesService {
  ...
  async findAll(tenantId: string) {
    ...
    const systemRoles = Object.entries(ROLE_PERMISSIONS).map(([name, permissions], index) => ({
      id: `SYS_${name}`, // Virtual ID
      name: name.replace('_', ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase()), // "HIRING_MANAGER" -> "Hiring Manager"
