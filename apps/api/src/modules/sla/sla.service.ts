...
export class SlaService {
  ...
  async calculateSlaStatus(applicationId: string): Promise<SLAStatus | null> {
    ...
    if (!slaLimit && application.job?.tenantId) {
      try {
        ...
        if (setting && setting.value) {
          const stageKey = this.getSlaKeyForStage(
    ...
    const stageEntryDate =
      application.activities[0]?.createdAt || application.appliedAt;
    const now = new Date();
  ...
  async checkSlas() {
    ...
    console.log(
      `SLA check complete: ${atRisk.length} at risk, ${overdue.length} overdue`,
    );
  }
  ...
  private async sendSlaNotification(
    ...
    try {
      ...
      if (application.job?.recruiterId)
        recipientIds.push(application.job.recruiterId);
      if (application.job?.hiringManagerId)
        recipientIds.push(application.job.hiringManagerId);
  ...
  private getSlaKeyForStage(stageName: string): string | null {
    ...
    if (name.includes("assessment") || name.includes("test"))
      return "assessment";
    if (name.includes("background") || name.includes("bgv"))
      return "backgroundCheck";
    if (name.includes("offer") && name.includes("acceptance"))
