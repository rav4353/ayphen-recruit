...
export class SuperAdminController {
  ...
  async impersonateUser(
    ...
    const result = await this.tenantsService.impersonate(
      userId,
      admin.id,
  ...
  async getMonitoringStats() {
    return {
      success: true,
      data: { status: "healthy", cpu: 45, memory: 60, disk: 30 },
    };
  }
  ...
  async getWebhooks(@Query("tenantId") tenantId?: string) {
    const webhooks = await this.apiManagementService.getWebhookSubscriptions({
      tenantId,
    });
  ...
  async createWebhook(@Body() dto: CreateWebhookDto, @Req() req: Request) {
    const admin = req.user as any;
    const webhook = await this.apiManagementService.createWebhookSubscription(
      dto,
      admin.id,
    );
  ...
  async processGDPRRequest(
    ...
    const user = req.user as { id: string };
    const result = await this.dataService.processGDPRRequest(
      id,
      action,
      user.id,
    );
