...
@Injectable()
export class SuperAdminGateway implements OnGatewayConnection, OnGatewayDisconnect {
    @WebSocketServer()
    server: Server;
    ...
    async handleConnection(client: AuthenticatedSocket) {
        try {
            ...
            const secret = this.configService.get<string>('SUPER_ADMIN_JWT_SECRET') || this.configService.get<string>('JWT_SECRET');
            ...
            this.logger.log(`Super Admin connected: ${client.id} (Admin: ${payload.sub})`);
        } catch (error) {
