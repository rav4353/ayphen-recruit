...
export class VideoMeetingService {
  ...
  private async getGoogleMeetConfig(
    tenantId: string,
  ): Promise<GoogleMeetConfig | null> {
    const setting = await this.prisma.setting.findUnique({
    ...
    return {
      providers: [
        ...
        { provider: "ZOOM", configured: !!zoomConfig?.clientId, name: "Zoom" },
        {
          ...
          configured: !!teamsConfig?.clientId,
  ...
  async createMeeting(
    ...
    switch (provider) {
      ...
      case "GOOGLE_MEET":
        return this.createGoogleMeetMeeting(
          tenantId,
          topic,
          startTime,
          endTime,
      ...
      default:
        throw new BadRequestException(
          `Unsupported video meeting provider: ${provider}`,
        );
    }
  }
  ...
  private async createZoomMeeting(
    ...
    if (!config?.clientId || !config?.clientSecret || !config?.accountId) {
      throw new BadRequestException(
        "Zoom is not configured. Please add credentials in Settings > Integrations.",
      );
    }
    ...
    try {
      ...
      return {
        ...
        dialInNumbers: meeting.settings?.global_dial_in_numbers?.map(
          (n: { country: string; number: string }) => ({
            country: n.country,
            number: n.number,
          }),
        ),
  ...
  private async createGoogleMeetMeeting(
    ...
    if (!config?.clientId || !config?.clientSecret || !config?.refreshToken) {
      throw new BadRequestException(
        "Google Meet is not configured. Please add credentials in Settings > Integrations.",
      );
    }
    ...
    try {
      ...
      return {
        ...
        dialInNumbers: dialInInfo
          ? [{ country: "US", number: dialInInfo.uri }]
          : undefined,
        startTime,
        endTime,
        topic,
  ...
  private async createTeamsMeeting(
    ...
    if (!config?.clientId || !config?.clientSecret || !config?.tenantId) {
      throw new BadRequestException(
        "Microsoft Teams is not configured. Please add credentials in Settings > Integrations.",
      );
    }
    ...
    try {
          ...
          ? [
              {
                country: "Dial-in",
                number:
                  meeting.audioConferencing.tollNumber ||
                  meeting.audioConferencing.dialinUrl,
              },
            ]
          : undefined,
      ...
      if (axios.isAxiosError(error)) {
        throw new BadRequestException(
          `Microsoft Graph API error: ${error.response?.data?.error?.message || error.message}`,
        );
      }
  ...
  private async deleteGoogleMeetMeeting(
    tenantId: string,
    eventId: string,
  ): Promise<void> {
    const config = await this.getGoogleMeetConfig(tenantId);
  ...
  private async deleteTeamsMeeting(
    tenantId: string,
    meetingId: string,
  ): Promise<void> {
    const config = await this.getTeamsConfig(tenantId);
