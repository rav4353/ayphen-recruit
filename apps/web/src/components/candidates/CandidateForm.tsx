...
export interface CandidateFormData {
    ...
    gdprConsent: boolean;
    resumeUrl?: string;
    candidateId?: string; // Optional manual override
...
export function CandidateForm({
    ...
    // Load next ID preview
    useEffect(() => {
        const fetchNextId = async () => {
            if (publicTenantId) return; // Don't show in public form
            try {
                const res = await candidatesApi.getNextId();
                setNextIdPreview(res.data?.data || res.data);
            } catch (error) {
                console.error('Failed to load next ID preview:', error);
            }
        };
        fetchNextId();
    }, [publicTenantId]);
            ...
            <form onSubmit={handleSubmit(async (data) => {
                ...
                <Card>
                    ...
                    <div className="p-6 pt-0 space-y-6">
                        ...
                        {nextIdPreview && (
                            <div className="bg-primary-50/50 dark:bg-primary-900/10 border border-primary-100 dark:border-primary-800/50 rounded-lg p-3 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    <span className="text-xs font-medium text-primary-600 dark:text-primary-400 uppercase tracking-wider">{t('candidates.fields.candidateId', 'Target Candidate ID')}</span>
                                    <span className="text-sm font-mono font-bold text-primary-700 dark:text-primary-300">{nextIdPreview}</span>
                                </div>
                                <span className="text-[10px] text-primary-500 dark:text-primary-500/70 italic">{t('candidates.autoGenerated', 'Auto-generated based on settings')}</span>
                            </div>
                        )}
