...
export function NotificationDrawer({ className }: NotificationDrawerProps) {
  ...
  return (
    <div ref={drawerRef} className={cn('relative', className)}>
      ...
      {/* Drawer Overlay - z-60 to cover sidebar (z-50) */}
      {isOpen && (
        <div className="fixed inset-0 bg-black/20 dark:bg-black/40 z-[60]" onClick={() => setIsOpen(false)} />
      )}

      {/* Drawer Panel */}
      <div
        className={cn(
          'fixed top-0 right-0 h-full w-full sm:w-96 bg-white dark:bg-neutral-900 shadow-2xl z-[70]',
